#include "curvature_editor_plugin.h"
...
BetterCurveEdit::BetterCurveEdit() {
	set_focus_mode(FOCUS_ALL);
	set_clip_contents(true);
}

void BetterCurveEdit::_bind_methods() {
	ClassDB::bind_method(D_METHOD("set_selected_index", "index"), &BetterCurveEdit::set_selected_index);
}

void BetterCurveEdit::set_curve(Ref<BetterCurve> p_curve) {
	if (p_curve == curve) {
		return;
	}

	if (curve.is_valid()) {
		curve->disconnect_changed(callable_mp(this, &BetterCurveEdit::_curve_changed));
		curve->disconnect(BetterCurve::SIGNAL_RANGE_CHANGED, callable_mp(this, &BetterCurveEdit::_curve_changed));
	}

	curve = p_curve;

	if (curve.is_valid()) {
		curve->connect_changed(callable_mp(this, &BetterCurveEdit::_curve_changed));
		curve->connect(BetterCurve::SIGNAL_RANGE_CHANGED, callable_mp(this, &BetterCurveEdit::_curve_changed));
	}
...
Ref<BetterCurve> BetterCurveEdit::get_curve() {
	return curve;
}

void BetterCurveEdit::set_snap_enabled(bool p_enabled) {
...
void BetterCurveEdit::set_snap_count(int p_snap_count) {
	snap_count = p_snap_count;
	queue_redraw();
	if (curve.is_valid()) {
		if (snap_count != BetterCurveEditor::DEFAULT_SNAP) {
...
Size2 BetterCurveEdit::get_minimum_size() const {
	return Vector2(64, MAX(135, get_size().x * ASPECT_RATIO)) * EDSCALE;
}

void BetterCurveEdit::_notification(int p_what) {
	switch (p_what) {
...
void BetterCurveEdit::gui_input(const Ref<InputEvent> &p_event) {
			...
			if (selected_index != -1) {
				if (selected_tangent_index == TANGENT_NONE) {
					...
					if (selected_tangent_index == TANGENT_LEFT) {
						...
						if (selected_index != (curve->get_point_count() - 1) && curve->get_point_right_mode(selected_index) != BetterCurve::TANGENT_LINEAR) {
						...
						if (selected_index != 0 && curve->get_point_left_mode(selected_index) != BetterCurve::TANGENT_LINEAR) {
...
void BetterCurveEdit::use_preset(int p_preset_id) {
	...
	switch (p_preset_id) {
		case PRESET_CONSTANT:
			...
			curve->set_point_right_mode(0, BetterCurve::TANGENT_LINEAR);
			curve->set_point_left_mode(1, BetterCurve::TANGENT_LINEAR);
			break;
	...
	undo_redo->create_action(TTR("Load BetterCurve Preset"));
...
void BetterCurveEdit::_curve_changed() {
	queue_redraw();
...
int BetterCurveEdit::get_point_at(Vector2 p_pos) const {
	if (curve.is_null()) {
		return -1;
	}
...
BetterCurveEdit::TangentIndex BetterCurveEdit::get_tangent_at(Vector2 p_pos) const {
...
float BetterCurveEdit::get_offset_without_collision(int p_current_index, float p_offset, bool p_prioritize_right) {
...
void BetterCurveEdit::add_point(Vector2 p_pos) {
	ERR_FAIL_COND(curve.is_null());
	...
	EditorUndoRedoManager *undo_redo = EditorUndoRedoManager::get_singleton();
	undo_redo->create_action(TTR("Add BetterCurve Point"));
...
void BetterCurveEdit::remove_point(int p_index) {
	ERR_FAIL_COND(curve.is_null());
	ERR_FAIL_INDEX_MSG(p_index, curve->get_point_count(), "BetterCurve point is out of bounds.");

	BetterCurve::Point p = curve->get_point(p_index);
	...
	EditorUndoRedoManager *undo_redo = EditorUndoRedoManager::get_singleton();
	undo_redo->create_action(TTR("Remove BetterCurve Point"));
...
void BetterCurveEdit::set_point_position(int p_index, Vector2 p_pos) {
	ERR_FAIL_COND(curve.is_null());
	ERR_FAIL_INDEX_MSG(p_index, curve->get_point_count(), "BetterCurve point is out of bounds.");
	...
	EditorUndoRedoManager *undo_redo = EditorUndoRedoManager::get_singleton();
	undo_redo->create_action(TTR("Modify BetterCurve Point"));
...
void BetterCurveEdit::set_point_tangents(int p_index, float p_left, float p_right) {
	ERR_FAIL_COND(curve.is_null());
	ERR_FAIL_INDEX_MSG(p_index, curve->get_point_count(), "BetterCurve point is out of bounds.");
	...
	EditorUndoRedoManager *undo_redo = EditorUndoRedoManager::get_singleton();
	undo_redo->create_action(TTR("Modify BetterCurve Point's Tangents"));
...
void BetterCurveEdit::set_point_left_tangent(int p_index, float p_tangent) {
	ERR_FAIL_COND(curve.is_null());
	ERR_FAIL_INDEX_MSG(p_index, curve->get_point_count(), "BetterCurve point is out of bounds.");
	...
	curve->set_point_left_tangent(p_index, initial_grab_left_tangent);
	EditorUndoRedoManager *undo_redo = EditorUndoRedoManager::get_singleton();
	undo_redo->create_action(TTR("Modify BetterCurve Point's Left Tangent"));
...
void BetterCurveEdit::set_point_right_tangent(int p_index, float p_tangent) {
	ERR_FAIL_COND(curve.is_null());
	ERR_FAIL_INDEX_MSG(p_index, curve->get_point_count(), "BetterCurve point is out of bounds.");
	...
	curve->set_point_right_tangent(p_index, initial_grab_right_tangent);
	EditorUndoRedoManager *undo_redo = EditorUndoRedoManager::get_singleton();
	undo_redo->create_action(TTR("Modify BetterCurve Point's Right Tangent"));
...
void BetterCurveEdit::toggle_linear(int p_index, TangentIndex p_tangent) {
	ERR_FAIL_COND(curve.is_null());
	ERR_FAIL_INDEX_MSG(p_index, curve->get_point_count(), "BetterCurve point is out of bounds.");
	...
	EditorUndoRedoManager *undo_redo = EditorUndoRedoManager::get_singleton();
	undo_redo->create_action(TTR("Toggle Linear BetterCurve Point's Tangent"));

	BetterCurve::TangentMode prev_mode = (p_tangent == TANGENT_LEFT) ? curve->get_point_left_mode(p_index) : curve->get_point_right_mode(p_index);
	BetterCurve::TangentMode mode = (prev_mode == BetterCurve::TANGENT_LINEAR) ? BetterCurve::TANGENT_FREE : BetterCurve::TANGENT_LINEAR;
...
void BetterCurveEdit::set_selected_index(int p_index) {
...
void BetterCurveEdit::update_view_transform() {
	...
	const Rect2 world_rect = Rect2(BetterCurve::MIN_X, min_y, BetterCurve::MAX_X, max_y - min_y);
...
Vector2 BetterCurveEdit::get_tangent_view_pos(int p_index, TangentIndex p_tangent) const {
	Vector2 dir;
	if (p_tangent == TANGENT_LEFT) {
...
Vector2 BetterCurveEdit::get_view_pos(Vector2 p_world_pos) const {
	return _world_to_view.xform(p_world_pos);
}

Vector2 BetterCurveEdit::get_world_pos(Vector2 p_view_pos) const {
...
template <typename T>
static void plot_curve_accurate(const BetterCurve &curve, float step, Vector2 scaling, T plot_func) {
	if (curve.get_point_count() <= 1) {
		...
		plot_func(Vector2(BetterCurve::MAX_X, last_point.y) * scaling - Vector2(1.5, 0), last_point * scaling, false);
...
struct CanvasItemPlotBetterCurve {
	CanvasItem &ci;
	Color color1;
	Color color2;

	CanvasItemPlotBetterCurve(CanvasItem &p_ci, Color p_color1, Color p_color2) :
			ci(p_ci),
			color1(p_color1),
			color2(p_color2) {}
...
void BetterCurveEdit::_redraw() {
	...
	CanvasItemPlotBetterCurve plot_func(*this, line_color, edge_line_color);
	...
	if (selected_index >= 0) {
		...
		if (grabbing == GRAB_NONE || initial_grab_pos == point_pos || selected_tangent_index != TANGENT_NONE) {
			...
			if (selected_index != 0) {
				...
				if (curve->get_point_left_mode(selected_index) == BetterCurve::TANGENT_FREE) {
				...
				// Hover indicator.
				if (hovered_tangent_index == TANGENT_LEFT || (hovered_tangent_index == TANGENT_RIGHT && !shift_pressed && curve->get_point_left_mode(selected_index) != BetterCurve::TANGENT_LINEAR)) {
			...
			if (selected_index != curve->get_point_count() - 1) {
				...
				// Square for linear mode, circle otherwise.
				if (curve->get_point_right_mode(selected_index) == BetterCurve::TANGENT_FREE) {
				...
				// Hover indicator.
				if (hovered_tangent_index == TANGENT_RIGHT || (hovered_tangent_index == TANGENT_LEFT && !shift_pressed && curve->get_point_right_mode(selected_index) != BetterCurve::TANGENT_LINEAR)) {
					draw_rect(Rect2(control_pos, Vector2(0, 0)).grow(tangent_hover_radius - Math::round(3 * EDSCALE)), tangent_color, false, Math::round(1 * EDSCALE));
				}
			}
		}
...
const int BetterCurveEditor::DEFAULT_SNAP = 10;

void BetterCurveEditor::_set_snap_enabled(bool p_enabled) {
	curve_editor_rect->set_snap_enabled(p_enabled);
	snap_count_edit->set_visible(p_enabled);
}

void BetterCurveEditor::_set_snap_count(int p_snap_count) {
	curve_editor_rect->set_snap_count(CLAMP(p_snap_count, 2, 100));
}

void BetterCurveEditor::_on_preset_item_selected(int p_preset_id) {
	curve_editor_rect->use_preset(p_preset_id);
}

void BetterCurveEditor::set_curve(const Ref<BetterCurve> &p_curve) {
	curve_editor_rect->set_curve(p_curve);
}

void BetterCurveEditor::_notification(int p_what) {
	switch (p_what) {
		case NOTIFICATION_THEME_CHANGED: {
			...
			p->add_icon_item(get_editor_theme_icon(SNAME("BetterCurveLinear")), TTR("Linear"), BetterCurveEdit::PRESET_LINEAR);
			p->add_icon_item(get_editor_theme_icon(SNAME("BetterCurveIn")), TTR("Ease In"), BetterCurveEdit::PRESET_EASE_IN);
			p->add_icon_item(get_editor_theme_icon(SNAME("BetterCurveOut")), TTR("Ease Out"), BetterCurveEdit::PRESET_EASE_OUT);
			p->add_icon_item(get_editor_theme_icon(SNAME("BetterCurveInOut")), TTR("Smoothstep"), BetterCurveEdit::PRESET_SMOOTHSTEP);
		} break;
		case NOTIFICATION_READY: {
			Ref<BetterCurve> curve = curve_editor_rect->get_curve();
			if (curve.is_valid()) {
...
BetterCurveEditor::BetterCurveEditor() {
	...
	snap_button->connect(SceneStringName(toggled), callable_mp(this, &BetterCurveEditor::_set_snap_enabled));
	...
	snap_count_edit->connect(SceneStringName(value_changed), callable_mp(this, &BetterCurveEditor::_set_snap_count));
	...
	presets_button->get_popup()->connect(SceneStringName(id_pressed), callable_mp(this, &BetterCurveEditor::_on_preset_item_selected));

	curve_editor_rect = memnew(BetterCurveEdit);
...
bool EditorInspectorPluginBetterCurve::can_handle(Object *p_object) {
	return Object::cast_to<BetterCurve>(p_object) != nullptr;
}

void EditorInspectorPluginBetterCurve::parse_begin(Object *p_object) {
	BetterCurve *curve = Object::cast_to<BetterCurve>(p_object);
	ERR_FAIL_NULL(curve);
	Ref<BetterCurve> c(curve);

	BetterCurveEditor *editor = memnew(BetterCurveEditor);
	editor->set_curve(c);
	add_custom_control(editor);
}

BetterCurveEditorPlugin::BetterCurveEditorPlugin() {
	Ref<EditorInspectorPluginBetterCurve> plugin;
	plugin.instantiate();
	add_inspector_plugin(plugin);

	EditorInterface::get_singleton()->get_resource_previewer()->add_preview_generator(memnew(BetterCurvePreviewGenerator));
}

///////////////////////

bool BetterCurvePreviewGenerator::handles(const String &p_type) const {
	return p_type == "BetterCurve";
}

Ref<Texture2D> BetterCurvePreviewGenerator::generate(const Ref<Resource> &p_from, const Size2 &p_size, Dictionary &p_metadata) const {
	Ref<BetterCurve> curve = p_from;
	if (curve.is_null()) {
